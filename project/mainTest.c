...
// Constants
#define MAX_NOTES 1           // Maximum number of notes on screen
...
// Clear the previous note position
void clear_note() {
...
void update_shape() {
  clear_note(); // Clear the previous note position
  if (note.active) {
    draw_note(); // Draw the current note only if active
  }
...
void wdt_c_handler() {
  ...
  check_hits();   // Check if the note was hit
  clear_note();
